...
glmm_cci_3 <- glmmTMB::glmmTMB(cci ~ tsp_ltd_s +
                                   tsp_scd_s +
                                   (1|tsp_comp),
                         family = glmmTMB::beta_family(link = "logit"),
                         data = full_data,
...
glmm_coi_3 <- glmmTMB::glmmTMB(coi ~ tsp_ltd_s +
                                   tsp_scd_s +
                                   (1|tsp_comp),
                         family = glmmTMB::beta_family(),
                         data = full_data,
...
cci_ltd_plot <- sjPlot::plot_model(
    ...
    dot.size = 4,
    line.size = 1.5,
    axis.title = c("TSP leaf trait dissimilarity (LTD)",
     "Crown complementarity index (CCI)"),
    title = "A"
    ) +
    scale_x_continuous(
...
cci_scd_plot <- sjPlot::plot_model(
    ...
    dot.size = 4,
    line.size = 1.5,
    axis.title = c("TSP Structural complexity dissimilarity (SCD)",
     ""),
    title = "B"
    ) +
    scale_x_continuous(
...
ggsave(file = path(figure_path, "figure_s8.svg"),
  plot = figure_s8,
  width=8,
  height=7)
```
...
ggsave(file = path(figure_path, "figure_s9.svg"),
    plot = figure_s9,
    width=14,
    height=7)
```
...
extra_coi_vs_height <- sjPlot::plot_model(
    lm_coi_ovlp2d,
    terms = "ovlp_2d [0:1]",
    type = "pred",
    show.data = TRUE,
...
ggsave(file = path(figure_path, "extra_coi_vs_height.svg"),
    plot = extra_coi_vs_height,
    width=7,
    height=7)
```
...
beech_glmm_cci_coi <- glmmTMB::glmmTMB(coi ~ cci +
                                        (1|dataset),
                                        family = glmmTMB::beta_family(),
                                        data = beech_data,
                                        REML = TRUE)
...
beech_glmm_cci_scd <- glmmTMB::glmmTMB(cci ~ scd +
                                        (1|dataset),
                                        family = glmmTMB::beta_family(),
                                        data = beech_data,
                                        REML = TRUE)
